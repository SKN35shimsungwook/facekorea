...
# 삼정(三停) — 얼굴을 상/중/하로 나눠 초년·중년·말년 운을 보는 전통 구획.
SAMJEONG = [
    ("상정(上停)", "이마 ~ 눈썹", "초년운(1~30세)을 상징한다고 전해져요. 넓고 반듯할수록 학업·윗사람 복이 좋다고 봐요."),
    ("중정(中停)", "눈썹 ~ 코끝", "중년운(31~50세)을 상징한다고 전해져요. 눈·코가 자리 잡은 구간이라 자수성가·재물운과 연결짓곤 해요."),
    ("하정(下停)", "코끝 ~ 턱끝", "말년운(51세~)을 상징한다고 전해져요. 턱이 두툼하고 안정적일수록 노후가 편안하다고 봐요."),
]

# 오악(五嶽) — 이마·코·좌우 광대·턱, 다섯 산에 비유한 전통 구획.
OAK = [
    ("이마(북악)", "지혜·초년운을 담당한다고 전해져요."),
    ("코(중악)", "재물운·자존감의 중심으로 본다고 전해져요."),
    ("좌 광대(동악)", "형제·동료운과 추진력을 상징한다고 전해져요."),
    ("우 광대(서악)", "대인관계·사회성을 상징한다고 전해져요."),
    ("턱(남악)", "말년운·포용력·부하운을 상징한다고 전해져요."),
]

# 십이궁(十二宮) — 얼굴의 12개 구역에 인생의 영역을 대응시키는 전통 관상 이론.
SIBIGUNG = [
    ("명궁(命宮)", "양 눈썹 사이(인당)", "타고난 성격과 전체 운의 중심 자리라고 전해져요. 넓고 매끈할수록 마음이 트여있다고 봐요."),
    ("재백궁(財帛宮)", "코, 특히 코끝(준두)", "재물을 모으고 쓰는 방식을 나타낸다고 전해져요."),
    ("형제궁(兄弟宮)", "눈썹", "형제자매·동료와의 인연을 나타낸다고 전해져요."),
    ("전택궁(田宅宮)", "눈두덩(윗눈꺼풀)", "부동산운과 가정의 안정감을 나타낸다고 전해져요."),
    ("남녀궁(男女宮)", "눈 밑(누당)", "자녀운과 애정의 결실을 나타낸다고 전해져요."),
    ("노복궁(奴僕宮)", "턱선·아래턱 양옆", "아랫사람 복과 통솔력을 나타낸다고 전해져요."),
    ("처첩궁(妻妾宮)", "눈꼬리(어미)", "배우자·연인과의 인연을 나타낸다고 전해져요."),
    ("질액궁(疾厄宮)", "미간 위 콧대 뿌리(산근)", "건강과 위기 대응력을 나타낸다고 전해져요."),
    ("천이궁(遷移宮)", "눈썹꼬리 바깥 이마 양끝", "이동·여행·환경 변화운을 나타낸다고 전해져요."),
    ("관록궁(官祿宮)", "이마 정중앙", "직업운과 사회적 명예를 나타낸다고 전해져요."),
    ("복덕궁(福德宮)", "눈썹 위 이마 양쪽", "타고난 복과 정신적 여유를 나타낸다고 전해져요."),
    ("부모궁(父母宮)", "이마 양쪽(일각·월각)", "부모와의 인연, 초년 배경운을 나타낸다고 전해져요."),
]
...
def analyze_to_prompt_dict(pts: np.ndarray) -> dict:
    """Gemini 프롬프트용 구조화 데이터 — 부위별 (제목, 설명)을 텍스트로 평탄화."""
    result = analyze(pts)
    return {part: f"{title} — {desc}" for part, (title, desc) in result.items()}
